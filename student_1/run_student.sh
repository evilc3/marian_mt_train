...
# Train
$MARIAN/marian -c /content/marian_mt_train/student_config_tiny.yml \
  ${compute} --workspace 9000 \
  --guided-alignment /content/marian_mt_train/corpus.aln.gz \
  --seed 1111 \
  --model model/model.npz \
  --train-sets /content/marian_mt_train/kde4_dataset/train.{$SRC,$TRG} \
  --vocabs model/vocab.$SRC$TRG.spm model/vocab.$SRC$TRG.spm \
  --dim-vocabs 32000 32000 \
  --valid-sets /content/marian_mt_train/kde4_dataset/valid.{$SRC,$TRG} \
...
cat /content/marian_mt_train/kde4_dataset/test.$SRC \
  | $MARIAN/marian-decoder \
      -c model/model.npz.best-bleu.npz.decoder.yml \
      ${compute} \
      --log evaluation/testset_decoding.log \
  | tee evaluation/testset_output.txt \
  | sacrebleu /content/marian_mt_train/kde4_dataset/test.$TRG ${SB_OPTS}

  # Run comet-score
  ./scripts/comet-score.sh evaluation/testset_output.txt /content/marian_mt_train/kde4_dataset/test.$SRC /content/marian_mt_train/kde4_dataset/test.$TRG
